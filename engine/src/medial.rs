...
pub fn medial_axis(region: &Polygons, min_w_mm: f64, max_w_mm: f64) -> Vec<ThickPolyline> {
    medial_axis_with_runts(region, min_w_mm, max_w_mm).0
}

/// [`medial_axis`], but also returning the RUNTS: the dangling sub-`2·max`
/// polylines the twig cull removes. Some are spurious corner ribs of a channel
/// that gets a real bead, some are genuinely tiny voids — the caller can tell
/// them apart by coverage and absorb the real ones into neighbouring beads.
pub fn medial_axis_with_runts(
    region: &Polygons,
    min_w_mm: f64,
    max_w_mm: f64,
) -> (Vec<ThickPolyline>, Vec<ThickPolyline>) {
    ...
    for attempt in 0..4u64 {
        ...
        match res {
            ...
            Ok(None) if attempt >= 1 => return (Vec::new(), Vec::new()),
            _ => {}
        }
    }
    (Vec::new(), Vec::new())
...
fn medial_axis_inner(
    region: &Polygons,
    min_w_mm: f64,
    max_w_mm: f64,
) -> Option<(Vec<ThickPolyline>, Vec<ThickPolyline>)> {
