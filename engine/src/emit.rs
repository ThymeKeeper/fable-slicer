...
pub fn to_gcode(layers: &[LayerPlan], s: &Settings) -> String {
    ...
    // Identity line, shaped for the host scanners: they read
    // "<slicer> <version> on <machine>" (Moonraker's identity regex wants the
    // " on " clause, and its parser is keyed to the name). No timestamp — the
    // machine it was sliced FOR is the useful half, and it keeps the same
    // model + settings emitting the same bytes every time, which is worth
    // more here than a clock.
    g.comment(&format!(
        "generated by Fable Slicer {} on {}",
        env!("CARGO_PKG_VERSION"),
        if s.printer_name.is_empty() { "an unnamed printer" } else { &s.printer_name },
    ));
...
mod tests {
    ...
    fn header_identity_matches_the_host_scanner_shape() {
        // Moonraker picks a metadata parser with
        //     r"(<known names>)\s(.*)\son"
        // against the header, so the identity line must read
        // "<name> <version> on <something>" or the whole card falls back to
        // the Z-move sniffer. Assert the SHAPE (the name list is the host's
        // to change — see docs/moonraker-metadata.md), and that it carries
        // the printer it was sliced for rather than a clock: same model +
        // settings must emit the same bytes twice.
        let m = mesh::Mesh::cube(10.0);
        let mut s = Settings::default();
        s.printer_name = "sovol-zero".into();
        let g = to_gcode(&generate(&m, &s), &s);
        let line = g.lines().find(|l| l.contains("generated by Fable Slicer")).expect("identity");
        let (name, rest) = line.split_once("Fable Slicer ").expect("name then version");
        assert!(name.ends_with("generated by "));
        let (version, machine) = rest.split_once(" on ").expect("' on ' clause the scanner needs");
        assert!(!version.is_empty() && version.chars().next().unwrap().is_ascii_digit());
        assert_eq!(machine, "sovol-zero");
        // Deterministic: no timestamp, so two runs of the same job match.
        assert_eq!(to_gcode(&generate(&m, &s), &s), g, "g-code must be reproducible");
        // A profile-less resolve still keeps the clause well-formed.
        let bare = Settings::default();
        let gb = to_gcode(&generate(&m, &bare), &bare);
        assert!(gb.contains("Fable Slicer") && gb.contains(" on an unnamed printer"));
    }
