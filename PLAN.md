...
- [~] **Multi-object scene / bed layout** (step 1 done): import multiple STLs, duplicate/delete, object list with selection, auto grid-arrange + drop-to-bed, slice & render all objects together. Each object = shared `Arc<Mesh>` + affine `mesh::Transform` (rotation/scale/translation); the combined scene is baked to one mesh in bed coords and the slicer's auto-centering is disabled (`Settings::auto_center_on_bed`) for explicit placement. **Next:** step 2 transforms (move/rotate/scale via panel + viewport gizmos), step 3 auto-orient (lay-flat / drop-to-bed).
