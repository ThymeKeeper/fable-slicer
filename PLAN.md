...
- **Independence:** clean reimplementation, not a fork. GPL/AGPL is acceptable;
  ...
  (One audited exception: `engine/src/skeletal.rs` is a structural port of
  CuraEngine's `SkeletalTrapezoidation`, attributed in its file header — see
  the decision log.)
...
| **Next action** | Auto-orient, variable layers. A real test print would beat all of it. (3MF import **done** 2026-06-12 — `mesh/src/threemf.rs`; Arachne graph walk **done** 2026-06-10 — `engine/src/skeletal.rs`, both defect layers fixed, see [docs/ARACHNE-GRAPH-WALK.md](docs/ARACHNE-GRAPH-WALK.md)) |
...
- [x] **Overhang wall slowdown** (2026-06-10, `plan.rs slow_overhanging_walls`): wall stretches whose centerline lies past the previous layer's outline (>½ bead unsupported — Orca's 50% bucket) become `PathKind::OverhangWall`: printed at `overhang_speed` (auto = bridge speed — 50 at defaults; same physics, beads onto air) with bridge-grade fan, at outer-wall acceleration. Loops split into consecutive open pieces at classification borders (no travel between), runs under 2·lw merged into their neighbour so the speed doesn't chatter; detection is a per-layer `difference(layer, prev⊕0.05)` that is empty (free) on most layers; vase mode skips (the spiral loop must stay whole). Hot-amber in the GUI preview + CLI SVG; `;TYPE:Overhang wall` in g-code. Benchy: 105 overhang stretches, 465 mm — the bow/stern undersides now print at 15 mm/s with full fan instead of 75–150
- [x] **Printer connection** (2026-06-10, new `printhost` crate): talk to the printer over Moonraker's HTTP API (the server every Klipper machine runs; Mainsail/Fluidd use the same endpoints). Printer-tier `host_url` + `api_key` (commented examples in voron24/sovol profiles); GUI **Connection** section (host, masked API key, Test connection) + **Send** / **Send & print** next to Slice/Export + a pause/resume/cancel/status row once a host is set — all network on a worker thread reporting into the status line (one op at a time, buttons disable). CLI: `--upload` / `--start-print` / `--host`. Blocking `ureq` client: multipart upload (`/server/files/upload` with the `print` field), print start/pause/resume/cancel, `/server/info` test, `print_stats` status. Tested against a one-shot in-process HTTP server (request shape, API-key header, urlencoding) and end-to-end against a mock Moonraker from both the CLI and the live GUI
- [x] **Bridge detection without supports + Orca bridge speed** (2026-06-10): the overhang scan now runs for `SupportMode::None` too — flat spans anchored on ≥2 sides print as straight `Bridge` lines (bridge speed/flow/fan) instead of ordinary fill sagging into air; true cantilevers stay normal fill (only supports or arc mode can rescue those). Arc mode unchanged: bridgeable islands bridge, the rest arc-fill — now as their own `PathKind::ArcOverhang`, since `bridge_speed` rose 15 → **50** (= Orca) and arcs physically can't ride that: each arc cantilevers sideways off the previous ring, so arcs derive ~30% of bridge speed clamped to the 5–15 mm/s window the technique works in (exactly the old behaviour at the new default). Internal bridges now derive ~110 mm/s (≈ Orca's measured ~118); overhang-wall auto follows bridge to 50. Benchy sovol+pla: **23m00s**, same-simulator 24.4 vs Orca's 29.3 min — bridging quality added *and* 1.3 min recovered
- [x] **Wipe on retract** (2026-06-10, `wipe_mm`, printer tier, default 2.0): after retracting, the nozzle drags back along the just-printed bead (walking the path tail backwards, capped at half the path; closed loops wipe back from the seam) before travelling — the pressure-release ooze smears onto plastic instead of blobbing the seam. Order matches Orca's default (full retract, then dry wipe). Carried across layers (a layer-change wipe runs one layer height above the bead — still releases pressure while moving). Estimator counts the wipe; Benchy: 479 of 483 retracts wiped, +1s. GUI slider under Retraction
- [x] **Aligned seams** (2026-06-10, `seam = "aligned"`): a real Prusa-style mode (the old "aligned" string was just an alias for nearest/rear) — `align_seams` walks layers bottom-up after ordering/centering and rotates every closed outer-wall loop to start at the vertex nearest the seam below it (10 mm track radius; new islands seed at the rear-most vertex), so the seam follows one continuous line even where the rear-most vertex would flip between competing features. Variable-width (arachne) loops rotate their widths with their points. Runs before travel planning so combing sees the final starts
- [x] **Internal bridges** (2026-06-10): the first solid layer over sparse infill prints as `PathKind::InternalBridge` — lines oriented **perpendicular to the sparse lines below** (each free span = one line spacing), extended half a bead into the surrounding solid to anchor, bridge-grade fan, flow 1.0 (anchored every cell; bridge under-extrusion would pinhole the surface). Speed is physics-derived, not another knob: free span = spacing/density, so `bridge_speed × (max_bridge_span / span)`, clamped to [bridge, solid] — ~110 mm/s at defaults (≈ Orca's measured internal-bridge speed) vs 50 for free-air bridges. Region = `solid[i] ∩ (inner[i−1] − solid[i−1])` from a precomputed per-layer solid split, morphologically opened; the solid boundary loop still rings the whole region (anchor rim), and the remaining solid fill excludes the bridged area. `;TYPE:Internal Bridge` (Orca's name), deep-teal in preview/SVG. Benchy: 168 bridge blocks; voron24 +1m06s — the cost of tops that don't sag into 15% infill
- [x] **Sovol Zero profile matched to Orca's high-speed numbers** (2026-06-10, pinned by a config test): interior accel + travel 40000 / outer wall 10000 / first layer auto→1000 (all = Orca's measured M204s), inner walls 400 mm/s, first layer 55, travel 1000, SCV 5. Basic `pla` carries Orca's generic-PLA melt ceiling (**21 mm³/s**, was 15) so flow clamping compares like for like; `pla-hf` (30) remains the stock high-flow hotend's real ceiling. Known divergence, documented in the profile: our external-wall auto stays at 50% of print speed (quality-first) where Orca asks 350 — pin "external mm/s" in the GUI for Orca's number. Benchy: **24m08s** (sovol+pla) / 32m44s (voron24+pla), vs 36m13s at the session start; same-simulator comparison vs Orca's file: 25.6 vs 29.6 min — the remaining gap is retraction economy (483 vs 2219 retracts, combed travel) and arachne having no gap-fill pass, partly given back to slower internal bridges and overhang walls
...
- [x] **3MF import** (2026-06-12, `mesh/src/threemf.rs`, zip + quick-xml): core-spec meshes/build/transforms/components plus the production extension's `p:path` cross-part references (the Orca/Bambu project layout); units scaled to mm; tolerant of junk indices/degenerates; support-type objects skipped. GUI imports each build item as its own scene object, **preserving the file's plate layout** when it fits the bed (grid-arrange fallback when it doesn't); CLI merges the build into one plate. Embedded slicer settings deliberately ignored — profiles are our own three-tier system. Verified against 24 real MakerWorld/Bambu files (up to 1M triangles, 31-object plates, unicode names): zero failures. `mesh --example dump_3mf` is the compatibility smoke tool.
...
- **Clean reimplementation, AGPL output.** Read CuraEngine (cleanest engine) and
  ...
  *Amended 2026-06-12 (provenance audit):* `engine/src/skeletal.rs` exceeded
  this policy — it is a **structural port** of CuraEngine's
  `SkeletalTrapezoidation` (same decomposition, ~15 non-paper constants,
  statement-level graph surgery, Cura-internal variable names), now attributed
  to UltiMaker in the file header per the PrusaSlicer precedent. Same license
  either way, so compliance is attribution-only — but it makes the AGPL choice
  load-bearing: relicensing permissively would require a true clean-room
  rewrite of that one file. Everything else audited independent
  (slice/wall/plan/fill/emit/arc, config/GUI text, profiles; all deps
  permissive — boostvoronoi BSL-1.0, clipper2 MIT/Apache-2.0).
...
## Reference reading map (port concepts, not code — exception: skeletal.rs, see decision log)
